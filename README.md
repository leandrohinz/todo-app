...
1. The traditional way is starting the apps one by one locally after getting all their dependencies.
2. You can create Docker images and deploy them using pure Docker and the docker cli.
3. The easiest way is to use `docker-compose` and deploy the apps locally.
4. You can also create a Kubernetes cluster and deploy the apps in pods, everything is up to you.

**Why don't you test all of them?**

 You will have another dependecy, [Zipkin](https://zipkin.io) which can be run using docker and is available on http://127.0.0.1:9411 by default.
